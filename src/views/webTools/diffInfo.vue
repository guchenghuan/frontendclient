<template>
  <div class="newwave">
    ...
    <div style="display: inline-block; float: right; margin-bottom: 10px">
      <span style="margin-right: 15px">分区选择</span>
      <el-select
        v-model="value"
        clearable
        placeholder="请选择分区"
...
export default {
  ...
  methods: {
      ...
      try {
        ...
        this.$http
          ...
          .then((res) => {
            if (res.msg === "success") {
              // 预处理数据
              if (res.firstresultdata && res.firstresultdata !== "no data") {
                this.selectfirst = res.firstresultdata;
              } else {
                this.selectfirst = [];
              }
              if (res.fenquresultdata && res.fenquresultdata !== "no data") {
                this.selectfenqu = res.fenquresultdata;
              } else {
                this.selectfenqu = [];
              }
              if (res.spaceresultdata && res.spaceresultdata !== "no data") {
                this.selectspace = res.spaceresultdata;
              } else {
                this.selectspace = [];
              }
              if (res.bofangresultdata && res.bofangresultdata !== "no data") {
                this.selectbofang = res.bofangresultdata;
              } else {
                this.selectbofang = [];
              }

              var bofangfail = this.resolvesucrate(res.bofangresultdata);
              var fenqufail = this.resolvesucrate(res.fenquresultdata);
              var spacefail = this.resolvesucrate(res.spaceresultdata);
              var firstfail = this.resolvesucrate(res.firstresultdata);

              var bofanglen = res.bofangresultdata.length;
              var fenqulen = res.fenquresultdata.length;
              var spacelen = res.spaceresultdata.length;
              var firstlen = res.firstresultdata.length;

              var bofangdiffratesdata = this.revesediffrate(
                res.bofangresultdata
              );
              var fenqudiffratesdata = this.revesediffrate(res.fenquresultdata);
              var spacediffratesdata = this.revesediffrate(res.spaceresultdata);
              var firstdiffratesdata = this.revesediffrate(res.firstresultdata);

              var bofangdomratesdata = this.revesedomrate(res.bofangresultdata);
              var fenqudomratesdata = this.revesedomrate(res.fenquresultdata);
              var spacedomratesdata = this.revesedomrate(res.spaceresultdata);
              var firstdomratesdata = this.revesedomrate(res.firstresultdata);

              var bofangobj = {
                pagecategory: "播放页",
                length:
                  res.bofangresultdata === "no data"
                    ? 0
                    : res.bofangresultdata.length,
                nums: 18,
                successrate:
                  bofangfail === 0
                    ? 100 + "%"
                    : this.resolenum((bofanglen - bofangfail) / bofanglen) +
                      "%",
                picrates:
                  bofangdiffratesdata[0] +
                  "%" +
                  " - " +
                  bofangdiffratesdata[bofangdiffratesdata.length - 1] +
                  "%",
                domrates:
                  bofangdomratesdata[0] +
                  "%" +
                  " - " +
                  bofangdomratesdata[bofangdomratesdata.length - 1] +
                  "%",
              };
              var fenquobj = {
                pagecategory: "分区页",
                length:
                  res.fenquresultdata === "no data"
                    ? 0
                    : res.fenquresultdata.length,
                nums: 26,
                successrate:
                  fenqufail === 0
                    ? 100 + "%"
                    : this.resolenum((fenqulen - fenqufail) / fenqulen) + "%",
                picrates:
                  fenqudiffratesdata[0] +
                  "%" +
                  " - " +
                  fenqudiffratesdata[fenqudiffratesdata.length - 1] +
                  "%",
                domrates:
                  fenqudomratesdata[0] +
                  "%" +
                  " - " +
                  fenqudomratesdata[fenqudomratesdata.length - 1] +
                  "%",
              };
              var spaceobj = {
                pagecategory: "个人中心",
                length:
                  res.spaceresultdata === "no data"
                    ? 0
                    : res.spaceresultdata.length,
                nums: 4,
                successrate:
                  spacefail === 0
                    ? 100 + "%"
                    : this.resolenum((spacelen - spacefail) / spacelen) + "%",
                picrates:
                  spacediffratesdata[0] +
                  "%" +
                  " - " +
                  spacediffratesdata[spacediffratesdata.length - 1] +
                  "%",
                domrates:
                  spacedomratesdata[0] +
                  "%" +
                  " - " +
                  spacedomratesdata[spacedomratesdata.length - 1] +
                  "%",
              };
              var firstobj = {
                pagecategory: "大首页",
                length:
                  res.firstresultdata === "no data"
                    ? 0
                    : res.firstresultdata.length,
                nums: 2,
                successrate:
                  firstfail === 0
                    ? 100 + "%"
                    : this.resolenum((firstlen - firstfail) / firstlen) + "%",
                picrates:
              ...
              this.sumaryinfo.push(bofangobj, fenquobj, spaceobj, firstobj);
              this.$message({
                message: "查询成功",
                type: "success",
              });
    ...
    changeselect() {
      ...
      if (this.value === 1) {
        this.diffinfo = this.selectfirst;
        var namevalue = "大首页";
      } else if (this.value === 2) {
        this.diffinfo = this.selectbofang;
        var namevalue = "播放页";
      } else if (this.value === 3) {
        this.diffinfo = this.selectfenqu;
        var namevalue = "分区页";
      } else {
        var namevalue = "个人中心";
        this.diffinfo = this.selectspace;
      }
      try {
        const obj = {
          wherename: namevalue,
